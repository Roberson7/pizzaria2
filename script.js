...
const updateAdminForm = () => {
  ...
  adminForm.innerHTML = `
        ...
        <div id="carousel-images">${carouselImages
          .map(
            (img, i) =>
  ...
  cartItems.innerHTML = cart
    .map((item) => {
      ...
      const displayName = item.isCustom
        ? `Pizza Personalizada (${item.slices.join(", ")})`
        : item.name;
      return `
            <div class="cart-item">
  ...
  carousel.innerHTML = carouselImages
    .map(
      (img, index) => `
        ...
        index + 1
      }" class="carousel-slide" style="width: ${100 / carouselImages.length}%;">
    `
    )
    .join("");
};
...
const openCustomPizzaModal = () => {
  ...
  selectedSlice = 0;
  document
    .getElementById("flavor-count")
    .addEventListener("change", updateFlavorCount);
...
const closeCustomPizzaModal = () => {
  ...
  document
    .getElementById("flavor-count")
    .removeEventListener("change", updateFlavorCount);
};

const updateFlavorCount = () => {
  const newFlavorCount = parseInt(
...
const populateFlavorSelect = () => {
  ...
  flavorSelect.innerHTML =
    ...
    pizzas
      .map(
        (pizza) =>
          `<option value="${pizza.name}">${pizza.name} - ${formatPrice(
            pizza.price
          )}</option>`
      )
      .join("");
  flavorSelect.onchange = (e) => {
    ...
    if (selectedFlavor && selectedSlice >= 0) {
      ...
      renderPizzaSVG();
      updateCustomPizzaSummary();
      selectedSlice = customPizzaSlices.includes(null)
        ? customPizzaSlices.indexOf(null)
        : -1;
      populateFlavorSelect();
...
const selectSliceForEdit = (index) => {
  selectedSlice = index;
  const flavorSelect = document.getElementById("flavor-select");
  flavorSelect.value = customPizzaSlices[index]
    ? `${customPizzaSlices[index]} - ${formatPrice(
        pizzas.find((p) => p.name === customPizzaSlices[index]).price
      )}`
    : "";
  const flavorImage = document.getElementById("selected-flavor-image");
...
const updateCustomPizzaSummary = () => {
  ...
  summary.innerHTML = `
        <strong>Sabores Escolhidos:</strong> ${
          customPizzaSlices.filter((s) => s).join(", ") || "Nenhum"
        }<br>
